...
<th></th>
			...
			<tbody>
				<?php 
					...
					$selectSales = $mysqli->query("SELECT * FROM sales WHERE sales_deleted = 0 && sales.sales_date BETWEEN '$from' AND '$to' ORDER BY sales_id DESC;
") or die("ERROR 5000");
...
<th></th>
			...
			<tbody>
				<?php 
					$selectSales = $mysqli->query("SELECT * FROM sales WHERE sales_deleted = 0 ORDER BY sales_id DESC") or die("ERROR 5000");
					while($data = $selectSales->fetch_assoc()):
				 ?>
				<tr class="item">
					<td><?php echo $data['sales_refNumber'];?></td>
